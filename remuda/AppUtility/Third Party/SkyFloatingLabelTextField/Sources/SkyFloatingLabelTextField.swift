...
open class SkyFloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    @objc dynamic open var titleFont: UIFont = .systemFont(ofSize: 16) {
        didSet {
            updateTitleLabel()
        }
    }
    ...
    */
    open var titleFormatter: ((String) -> String) = { (text: String) -> String in
        if #available(iOS 9.0, *) {
            return text.firstLetterUppercased()
        } else {
            return text.firstLetterUppercased()
        }
    }
    ...
    */
    ...
    override open func editingRect(forBounds bounds: CGRect) -> CGRect {
        ...
        return isTitleVisible() ? rect : centeredFrame(forBounds: bounds)
    }
...
//        )
//        return rect
        centeredFrame(forBounds: bounds)
    }
    
    private func centeredFrame(forBounds bounds: CGRect) -> CGRect {
        var frame = bounds
